...
import { SERVICE_TYPE_LIST_ENDPOINT, } from '../endpoints.names';
import { FETCH_SERVICE_TYPE_LIST } from '../actions.names';
...
const actions: ActionTree<CoreState, RootState> = {
    async [FETCH_SERVICE_TYPE_LIST]({ rootState, commit }): Promise<any> {
        return new Promise((resolve, reject) => {
            axios.get(SERVICE_TYPE_LIST_ENDPOINT, generateAuthHeader(rootState.AuthModule.access_token)).then(({ data }) => {
                resolve(data);
            }).catch(e => {
                reject(e);
            });
